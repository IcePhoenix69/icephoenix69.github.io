...
function onDeleteLastAddedItem() {
  ...
  //
  let removedElem = todoList.pop();
  for (let i = 0; i < 1; i++) {
    console.log('Element removed:')
    console.log(removedElem)
    console.log('Rest of the list:')
...
function onHideDoneTaskElemClick() {
  ...
  // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/for...of
    let newArrayforShow = []
    let i = 0;
  for (let checked of todoList){
    if (checked.done === false){
      newArrayforShow.push(todoList[i])
    }
    i++;
  }
  todoList = newArrayforShow; // passo gli elementi dell'array creato dentro TodoList (che sono gli stessi ma filtrati)
  console.log(newArrayforShow)
  refreshTodolistElements()
}
...
function onShowDoneAtTheBottomClick() {
  ...
  /*
  let newArrayforTrue = [];
  let newArrayforFalse = [];
  let i=0;
  for (let task of todoList){
    if (task.done === true){
      newArrayforTrue.push(todoList[i])
    } else {
      newArrayforFalse.push(todoList[i])
    }
  }
  console.log(newArrayforTrue);
  console.log(newArrayforFalse);
  todoList.sort((true, false) => true - false);*/
  refreshTodolistElements()
}
...
function onShowOnlyShortTasks() {
  ...
  let filter = []
  for (let task of todoList){
    if (task.textTodo.length <= 10){
      filter.push(todoList[i])
    }
    i++;
  }
